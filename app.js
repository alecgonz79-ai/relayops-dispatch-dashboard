...
async function copyMorningAppsScript() {
  let code='';
  try { const response=await fetch(`${MORNING_APPS_SCRIPT_URL}?v=20260711-main-ops-log-fixed-layout`,{cache:'no-store'});if(response.ok)code=await response.text(); } catch(error) {}
...
async function testMorningSheetsConnector() {
  ...
  try {
    ...
    if(!/2026-07-11-main-ops-log-fixed-layout/.test(text))throw new Error('Connector deployment is outdated. Replace the Apps Script with the revised OPS LOG 2026 fixed-layout version, then choose Deploy → Manage deployments → Edit → New version → Deploy.');
    state.morningSheetsLastError='';
    persist(); render();
