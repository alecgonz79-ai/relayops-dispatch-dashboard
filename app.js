...
function refreshFleetStatus() {
  if(state.fleetImport?.vehicles?.length) {
    ...
    toast(`${state.fleetUpdateSummary.updated} changed · ${total} EV cards refreshed from latest fleet upload`);
    return;
  }
...
function downloadFleetTemplate(){const h=['Vehicle Name','VIN','License Plate','Active','Operational Status','Battery %','Range Miles'];const amazon=['LLOL EV 21','7FCEHEB79PN014816','9ABC123','Active','Operational','',''];const fleetos=['','7FCEHEB79PN014816','','','','63%','98'];downloadBlob(`${h.join(',')}\r\n${amazon.map(csvEscape).join(',')}\r\n${fleetos.map(csvEscape).join(',')}\r\n`,'text/csv','fleetos-amazon-ev-import-template.csv');toast('FleetOS/Amazon EV template downloaded');}
