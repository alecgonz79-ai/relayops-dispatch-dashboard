...
function morningSheetsHandoffProof(payload=morningSheetsConnectorPayload()) {
  ...
  const exactRows=rows.length===visibleRows.length&&rows.every((row,i)=>JSON.stringify(row.map(cell=>String(cell??'')))===JSON.stringify((visibleRows[i]||[]).map(cell=>String(cell??''))));
  ...
  return {ready:exactRows&&rows.length>0,rows:rows.length,visibleRows:visibleRows.length,range:rows.length?`A3:M${lastRow}`:'A3:M',dividers:separatorRows.length,dividerRows:separatorRows.join(', '),sections:sections.length,mergeRows};
}
