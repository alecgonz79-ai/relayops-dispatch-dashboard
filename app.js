...
function fleetGapAuditStrip() {
  ...
  const preview=rows.slice(0,5).map(row=>{
    const [issue,vin,name,,,,,,fix]=row;
    return `<span><b>${esc(issue)}</b>${esc(vin||name||'Expected count issue')}<em>${esc(fix)}</em></span>`;
  }).join('');
...
function fleetRefreshReadinessStrip() {
  ...
  return `<div class="fleet-refresh-readiness ${tone}"><div><strong>Refresh readiness</strong><span>${esc(message)}</span></div><div><div class="refresh-source-pills">${fleetSourcePill('amazon','Amazon')}${fleetSourcePill('fleetos','FleetOS')}</div><div class="refresh-update-map"><span><b>Amazon</b>name + plate + active/grounded</span><span><b>FleetOS</b>battery % + range</span><span><b>VIN</b>match key</span></div></div><button class="btn small ${hasAmazon&&hasFleetos?'lime':'primary'}" data-action="${hasAmazon&&hasFleetos?'refresh-fleet':'fleet-import'}">${hasAmazon&&hasFleetos?'Review refresh':'Upload missing source'}</button></div>`;
}
...
function fleetNameSource(vehicle={}) {
  const source=String(vehicle.source||'').toLowerCase();
  if(source.includes('amazon fleet list'))return {className:'ok',label:'Amazon name verified',detail:'Official name from Amazon fleet list'};
  if(source.includes('fleetos tracker')||source.includes('rivian'))return {className:'warn',label:'Name needs Amazon',detail:'Temporary name until Amazon fleet list is uploaded'};
  return {className:'demo',label:'Demo name',detail:'Demo/sample name until portal uploads are added'};
}
