...
function morningWaveGroup(section,sectionIndex=0) {
  ...
  const separatorRow=`<tr class="wave-separator">${showGridLabels?`<th class="sheet-row-num">${rowBase+rows.length+1}</th>`:''}<td colspan="13"></td></tr>`;
...
function fleetUploadPrepChecklist() {
  return `<div class="fleet-upload-prep"><div><strong>Before you press refresh</strong><span>Use fresh files from both portals so the EV board can be trusted for dispatch.</span></div><ol><li><b>1</b><span>Amazon fleet list file</span><small>From logistics.amazon.com/fleet-management/#vehicles — needs Vehicle Name, VIN, Plate, Active/Inactive, Operational/Grounded.</small></li><li><b>2</b><span>FleetOS tracker file</span><small>From business.rivian.com/vehicles/tracker — needs VIN, battery %, and range miles.</small></li><li><b>3</b><span>Upload both together</span><small>RelayOps matches the VIN, keeps Amazon’s exact van name, then updates FleetOS battery/range.</small></li></ol></div>`;
}
