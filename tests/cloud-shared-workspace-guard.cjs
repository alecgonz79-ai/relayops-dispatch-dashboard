...
if(!source.includes('function canInitialize(){return canWrite()&&operationDateIsWritable(operationDate());}'))throw new Error('An automatic shared-link dispatcher must initialize only a non-expired shared day');
