...
const checks = `
  ...
  if(!rosterSchedule.includes('Drivers on route')||!rosterSchedule.includes('All scheduled driver shifts')||!rosterSchedule.includes('Dispatcher shifts')||!rosterSchedule.includes('Other scheduled shifts')||!rosterSchedule.includes('Fleet Coordinator/Rescue')||!rosterSchedule.includes('Warehouse Support')) throw new Error('Minimal Paycom Opening Roster sections missing');
  state.morningRoutes=savedQueueMorning;state.scheduleEntries=savedScheduleEntries;state.morningOperationDate=savedOperationDate;state.messageQueueStatus={};state.modal=null;state.pendingDriverText=null;
