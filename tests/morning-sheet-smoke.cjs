...
const checks = `
  ...
  refreshFleetStatus();
  if (state.modal !== 'fleet-refresh' || !state.fleetRefreshPreview || state.fleetRefreshPreview.expectedShort !== 1 || state.fleetRefreshPreview.fleetosShort !== 1 || !state.fleetRefreshPreview.blockers.some(x => x.includes('FleetOS battery row')) || !state.fleetRefreshPreview.blockers.length || !state.fleetRefreshPreview.fieldBreakdown?.length || !modal().includes('Approve fleet refresh') || !modal().includes('WAIT') || !modal().includes('Approval summary') || !modal().includes('blockers before approval') || !modal().includes('Amazon status/name changes') || !modal().includes('FleetOS battery/range changes') || !modal().includes('need review before approving') || !modal().includes('source rows read') || !modal().includes('short of expected') || !modal().includes('FleetOS battery coverage short') || !modal().includes('1/2') || !modal().includes('not in upload') || !modal().includes('Fix before approving') || !modal().includes('1 EV short of expected') || !modal().includes('1 FleetOS battery row missing') || !modal().includes('Refresh will update') || !modal().includes('Changed fields by source') || !modal().includes('Active / Inactive') || !modal().includes('Operational / Grounded') || !modal().includes('Battery %') || !modal().includes('Range miles') || !modal().includes('Amazon fleet list') || !modal().includes('Amazon wins for van names and status') || !modal().includes('FleetOS tracker') || !modal().includes('FleetOS wins for battery and range') || !modal().includes('official EV name') || !modal().includes('battery %, range miles') || !modal().includes('Accuracy reminder') || !modal().includes('upload fresh exports first before approving') || !modal().includes('warn-approve') || !modal().includes('Approve anyway')) throw new Error('Fleet refresh should show approval preview before applying');
  resetFleetDemo();
  ...
  state.fleetExpectedCount = 0;
  refreshFleetStatus();
  if (!state.fleetRefreshPreview || state.fleetRefreshPreview.blockers.length || !modal().includes('SAFE') || !modal().includes('Approval summary') || !modal().includes('Ready to approve after review') || !modal().includes('Safe to approve') || !modal().includes('Approve refresh') || modal().includes('Approve anyway')) throw new Error('Clean fleet refresh should show normal approval button');
...
const fleetCss = fs.readFileSync('styles.css','utf8');
  ...
  if (!fleetCss.includes('minmax(74px,1fr)') || !fleetCss.includes('min-height:42px') || !fleetCss.includes('width:19px; height:10px') || !fleetCss.includes('repeat(7,minmax(0,1fr))') || !fleetCss.includes('repeat(4,minmax(0,1fr))') || !fleetCss.includes('.fleet-card-cue') || !fleetCss.includes('.fleet-source-map') || !fleetCss.includes('.fleet-source-timestamps') || !fleetCss.includes('.refresh-freshness-summary') || !fleetCss.includes('.rivian-id-summary') || !fleetCss.includes('.fleet-gap-fix-tips') || !fleetCss.includes('.fleet-refresh-blockers') || !fleetCss.includes('.fleet-refresh-guide') || !fleetCss.includes('.name-source-pill em') || !fleetCss.includes('.gap-priority') || !fleetCss.includes('.change-source-pills') || !fleetCss.includes('.warn-approve')) throw new Error('Tiny EV grid should stay compact and scan-friendly');
