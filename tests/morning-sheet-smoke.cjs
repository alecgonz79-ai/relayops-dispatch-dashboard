...
const checks = `
  ...
  const paycomText=['Sun','7/5','JOSE LOPEZ RAMIREZ','7:30 AM - 4:00 PM','First Opening Dispatcher','Sun','7/5','STEVEN SCICCHITANO','8:00 AM - 4:30 PM','Fleet Coordinator/Resuce','Sun','7/5','FREDY GUERRA','12:00 PM - 8:30 PM','Midshift','Sun','7/5','GERARDO GODINEZ','2:00 PM - 10:30 PM','Closing Dispatcher','Sun','7/5','WAREHOUSE PERSON','9:00 AM - 5:00 PM','Warehouse Support'].join('\\n');
  ...
  if(state.scheduleEntries.length!==5||state.scheduleEntries[1].role!=='Fleet Coordinator/Rescue'||scheduleRoleGroup(state.scheduleEntries[0].role)!=='dispatch'||scheduleRoleGroup(state.scheduleEntries[2].role)!=='driver'||scheduleRoleGroup(state.scheduleEntries[4].role)!=='other') throw new Error('Paycom schedule text classification failed');
  ...
  if(!filteredRoster.includes('Fredy Guerra')||!filteredRoster.includes('1 shown · 1 total')||!filteredRoster.includes('data-schedule-filter')) throw new Error('Paycom shift filter did not isolate matching shifts');
  state.scheduleFilter='all';openRosterSwap('CX778','Maya Collins','calloff');
  if(state.modal!=='roster-swap'||!modal().includes('Call off & replace')||!modal().includes('Fredy Guerra')||!modal().includes('removed from today’s text queue')) throw new Error('Call-off replacement review modal missing');
  const originalGetElementById=document.getElementById;document.getElementById=id=>id==='roster-swap-replacement'?{value:'Fredy Guerra'}:originalGetElementById(id);applyRosterSwap();document.getElementById=originalGetElementById;
  if(state.morningRoutes[0].driver!=='Fredy Guerra + Vanessa Balderama'||!state.callOffDriverKeys[callOffStatusKey('Maya Collins')]||morningMessageQueueRows().some(row=>row.name==='Maya Collins')) throw new Error('Call-off swap did not update Morning Sheet or text queue');
  state.morningRoutes=savedQueueMorning;state.scheduleEntries=savedScheduleEntries;state.morningOperationDate=savedOperationDate;state.messageQueueStatus={};state.callOffDriverKeys={};state.scheduleFilter='all';state.modal=null;state.pendingDriverText=null;state.pendingRosterSwap=null;
