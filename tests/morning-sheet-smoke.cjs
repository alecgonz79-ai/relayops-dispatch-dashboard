...
const checks = `
  ...
  const stayHomeRoster=openingRosterScheduleHtml();if(!state.scheduleStayHome[scheduleDriverMarkKey('Casey Nolan')]||!stayHomeRoster.includes('Told To Stay Home')||stayHomeRoster.indexOf('data-roster-name="casey nolan"')<stayHomeRoster.indexOf('Told To Stay Home')) throw new Error('Told To Stay Home did not remove the driver from PAYCOM and add the dated status lane');
  restoreRosterStatus('Casey Nolan','stay-home');addRosterReduction('Casey Nolan','Modified Duty');
  const sharedRosterStatus=sharedWorkspaceState();if(!state.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||!sharedRosterStatus.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||!openingRosterScheduleHtml().includes('Reductions')||!openingRosterScheduleHtml().includes('Open Paycom login')||!openingRosterScheduleHtml().includes('VTO 2 · 1')||!openingRosterScheduleHtml().includes('VTO 4 · 0')) throw new Error('Reduction, Paycom shortcut, backup totals, or shared status persistence are missing');
  ...
  const savedPicklistMorning=state.morningRoutes,savedCalloffReasons=state.callOffReasons,savedPicklistTopics=state.openingPicklistTopics,savedPicklistNotes=state.openingPicklistNotes,savedPicklistCalloffRows=state.openingPicklistCalloffRows,savedPicklistTopicRows=state.openingPicklistTopicRows,savedPicklistDrafts=state.openingPicklistCalloffDrafts,savedPicklistBackups=state.openingPicklistBackupOverrides,savedPicklistLabels=state.openingPicklistLabels;
  state.morningRoutes=[
  ...
  ];state.callOffReasons={[callOffStatusKey('Maya Collins')]:'No car'};state.openingPicklistTopics=['Safety first','New policy','',''];state.openingPicklistNotes='Bring keys to stand up';
  ...
  if(!picklistHtml.includes('Wave One Driver')||!picklistHtml.includes('CX101')||!picklistHtml.includes('STG.V.1')||!picklistHtml.includes('Back Ups')||!picklistHtml.includes('VTO 2')||!picklistHtml.includes('data-picklist-backup="vto2:0"')||!picklistHtml.includes('CALL OFF')||!picklistHtml.includes('No car')||!picklistHtml.includes('STAND UP TOPICS')||!picklistHtml.includes('Bring keys to stand up')||!picklistHtml.includes('Print the Opening Picklist')||!picklistHtml.includes('More picklist tools')||!picklistHtml.includes('Add more call-off lines')||!picklistHtml.includes('Add more stand-up topics')||!picklistHtml.includes('data-picklist-date')) throw new Error('Opening Picklist data, editable backups, tools, dated notes, right-side worksheet, or print action is incomplete');
  ...
  const beforeCalloffRows=state.openingPicklistCalloffRows,beforeTopicRows=state.openingPicklistTopicRows;action('add-picklist-calloff-row',{});action('add-picklist-topic-row',{});if(state.openingPicklistCalloffRows!==beforeCalloffRows+1||state.openingPicklistTopicRows!==beforeTopicRows+1)throw new Error('Expandable call-off or stand-up topic rows did not grow');
  ...
  state.morningRoutes=savedPicklistMorning;state.callOffReasons=savedCalloffReasons;state.openingPicklistTopics=savedPicklistTopics;state.openingPicklistNotes=savedPicklistNotes;state.openingPicklistCalloffRows=savedPicklistCalloffRows;state.openingPicklistTopicRows=savedPicklistTopicRows;state.openingPicklistCalloffDrafts=savedPicklistDrafts;state.openingPicklistBackupOverrides=savedPicklistBackups;state.openingPicklistLabels=savedPicklistLabels;
  state.morningRoutes=savedQueueMorning;state.scheduleEntries=savedScheduleEntries;state.morningOperationDate=savedOperationDate;state.messageQueueStatus={};state.callOffDriverKeys={};state.scheduleDriverMarks={};state.scheduleStayHome={};state.scheduleReductions={};state.scheduleFilter='all';state.modal=null;state.pendingDriverText=null;state.pendingRosterSwap=null;
