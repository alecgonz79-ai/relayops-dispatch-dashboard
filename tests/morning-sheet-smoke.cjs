...
const checks = `
  ...
  const pdfDriverContacts=driverContactsFromText([
    'Name and ID Position Personal Phone Number Status',
    'Adrian Anthony Elizondo A32HN7H6JNEAH4 Helper, Driver AMZL_HELPER, EDV (951) 472-9383 alelizondo@example.com Active',
    'Vanessa Balderama',
    'A20T7SGM66WFFR',
    '(840) 241-8056'
  ].join('\\n'));
  if(pdfDriverContacts.length!==2||pdfDriverContacts[0].name!=='Adrian Anthony Elizondo'||pdfDriverContacts[0].phone!=='(951) 472-9383'||pdfDriverContacts[0].transporterId!=='A32HN7H6JNEAH4'||pdfDriverContacts[1].name!=='Vanessa Balderama'||pdfDriverContacts[1].phone!=='(840) 241-8056'||pdfDriverContacts[1].transporterId!=='A20T7SGM66WFFR') throw new Error('Driver PDF text parser did not match names, phones, or transporter IDs');
  mergeDriverContacts(driverContacts);
  state.page = 'team';
  const teamHtml = teamPage();
  if (!teamHtml.includes('Driver & phone import') || !teamHtml.includes('Open Amazon Workforce') || !teamHtml.includes('logistics.amazon.com/workforce?pageId=da_console_associates') || !teamHtml.includes('Import Drivers CSV / Excel / PDF') || !teamHtml.includes('Choose CSV or Excel / PDF') || !teamHtml.includes('text-based PDF') || !teamHtml.includes('Add Delivery Associate') || !teamHtml.includes('(555) 123-4567') || !teamHtml.includes('Vanessa Balderama') || !teamHtml.includes('Future text reminder prep') || !teamHtml.includes('secure SMS connector') || !teamHtml.includes('Contacts are never embedded in the public website') || !teamHtml.includes('protected station workspace') || !teamHtml.includes('driver-delete-button') || !teamHtml.includes('request-driver-removal') || !teamHtml.includes('data-driver-card-toggle="true"') || teamHtml.includes('>Active</span>')) throw new Error('Drivers & Team Workforce import UI missing CSV/Excel/PDF import, expandable cards, deletion, manual add, shared-contact privacy, names, phones, Amazon link, or SMS guidance');
  ...
  if(!supportQueue.some(row=>row.name==='Vanessa Balderama'&&row.source==='Helper / Ad hoc')||supportQueue.some(row=>row.name==='Fredy Guerra')) throw new Error('Helpers must stay in the text queue while Midshift support remains outside the route-driver queue');
  ...
  state.scheduleFilter='rescue';const filteredRoster=openingRosterScheduleHtml();
  if(!filteredRoster.includes('Vanessa Balderama')||!filteredRoster.includes('1 shown of 2 scheduled')||!filteredRoster.includes('data-schedule-filter')||!filteredRoster.includes('Fredy Guerra')||scheduleRoleGroup('Midshift')!=='other') throw new Error('Paycom shift filter or Midshift-to-Other classification is incorrect');
  ...
  if(state.scheduleDriverMarks[scheduleDriverMarkKey('Fredy Guerra')]==='adhoc'||state.morningRoutes.some(row=>row.driver==='Fredy Guerra'&&row.wave==='Ad hoc')||!openingRosterScheduleHtml().includes('Other scheduled shifts')) throw new Error('Midshift was incorrectly allowed into Adhoc instead of staying in Other Scheduled Shifts');
  ...
  const stayHomeRoster=openingRosterScheduleHtml(),stayHomeStats=driverStayHomeStats('Casey Nolan'),stayHomeSectionStart=stayHomeRoster.indexOf('scheduled-section stay-home');if(!state.scheduleStayHome[scheduleDriverMarkKey('Casey Nolan')]||stayHomeStats.count!==2||!stayHomeRoster.includes('Told To Stay Home')||!stayHomeRoster.includes('2 times in the last 14 days')||stayHomeRoster.lastIndexOf('data-roster-name="casey nolan"')<stayHomeSectionStart) throw new Error('Told To Stay Home did not save the dated two-week fairness history, remain in All Scheduled, or add the driver to its status lane');
  ...
  const sharedRosterStatus=sharedWorkspaceState(),persistentRosterStatus=persistentWorkspaceState(),reductionRoster=openingRosterScheduleHtml();if(!state.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||!sharedRosterStatus.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||sharedRosterStatus.scheduleStayHomeHistory||!persistentRosterStatus.scheduleStayHomeHistory['2026-06-28|casey nolan']||!reductionRoster.includes('Reductions')||reductionRoster.includes('data-driver-name="Casey Nolan" data-driver-role="Modified Duty">Move to backup drivers')||!reductionRoster.includes('Open Paycom login')||!reductionRoster.includes('VTO 2 · 1')||!reductionRoster.includes('VTO 4 · 0')) throw new Error('Modified Duty reduction safety, Paycom shortcut, backup totals, or station-persistent fairness history are missing');
  moveReductionToBackup('Casey Nolan','Modified Duty');if(!state.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||state.scheduleDriverMarks[scheduleDriverMarkKey('Casey Nolan')]==='backup'||state.scheduleBackupRecords[scheduleDriverMarkKey('Casey Nolan')]||openingPicklistBackupRows().some(row=>row.name==='Casey Nolan'))throw new Error('Modified Duty driver was incorrectly moved from Reductions into a backup list');
  ...
  const savedHelperEntries=state.scheduleEntries,savedHelperMorning=state.morningRoutes,savedHelperMarks=state.scheduleHelpers,savedHelperEquipment=state.equipmentImport,savedHelperCustomRows=state.deviceCustomRows,savedDriverAliases=state.driverNameAliases;
  ...
  openHelperMatch('Helper Person');if(!modal().includes('Short helper name')||!modal().includes('original identity remains'))throw new Error('Helper match modal is missing the custom abbreviation option');document.getElementById=id=>id==='helper-match-driver'?{value:'helper-main-1'}:id==='helper-display-name'?{value:'H. Person'}:originalGetElementById(id);applyHelperMatch();document.getElementById=originalGetElementById;
  ...
  if(driverDisplayValue(helperMain.driver)!=='Route Driver + H. Person'||canonicalDriverValue('Route Driver + H. Person')!=='Route Driver + Helper Person'||morningSheetsConnectorPayload().rows.find(row=>row[2]==='CX901')?.[1]!=='Route Driver + H. Person')throw new Error('Custom helper abbreviation must display/export while preserving the canonical helper identity');
  if(sharedWorkspaceState().driverNameAliases||!persistentWorkspaceState().driverNameAliases['helper person'])throw new Error('Driver/helper aliases must sync universally through station-persistent state');
  ...
  unmatchHelper('Helper Person');if(helperMain.driver!=='Route Driver'||helperMain.helperBag||!helperMain.missingHelper||routeDriverDisplayValue(helperMain)!=='Route Driver + ?'||!state.morningRoutes.some(row=>row.route==='HELPER'&&row.missingHelper&&routeDriverDisplayValue(row)==='?')||state.scheduleHelpers[scheduleHelperKey('Helper Person')]?.matchedRoute||!openingRosterScheduleHtml().includes('Match with Driver'))throw new Error('Un-matching a Helper did not preserve the route, show the missing helper placeholder, and return the Helper card');
  state.scheduleEntries=savedHelperEntries;state.morningRoutes=savedHelperMorning;state.scheduleHelpers=savedHelperMarks;state.equipmentImport=savedHelperEquipment;state.deviceCustomRows=savedHelperCustomRows;state.driverNameAliases=savedDriverAliases;
  ...
  const morningTemplateSections=morningSections(state.morningRoutes),expectedMorningLabels=['WAVE 1','WAVE 2','WAVE 3','WAVE 4','WAVE 5','WAVE 6',"ADHOC's",'HELPERS','DSP'],expectedMorningCapacities=[15,15,15,15,15,15,15,15,6];
  if(morningTemplateSections.length!==9||morningTemplateSections.some((section,index)=>section.label!==expectedMorningLabels[index]||section.routeCapacity!==expectedMorningCapacities[index])) throw new Error('Morning Sheet must preserve the exact six-wave, Adhocs, Helpers, and DSP capacities of the 142-row template');
  const picklistSections=openingPicklistSections(),picklistHtml=openingPicklistHtml(),picklistRoster=rosterPage();
  const expectedWaveCapacities=[15,15,15,15,15,15];
  ...
  if(fifteenSection.rows.length!==15||fifteenSection.capacity!==15||!fifteenHtml.includes('rowspan="15"')||!fifteenHtml.includes('rowspan="16"')||!fifteenHtml.includes('Driver 15'))throw new Error('A full 15-driver Wave 1 Picklist section must keep its Wave/Pad cells merged through the final driver row');
  ...
  state.organizationName='Legacy Logistics';state.stationCode='DJT6';state.adminPinUnlocked=true;
  ...
  const trainingHtml=performancePage();
  if(!trainingHtml.includes('DA Performance Insights')||!trainingHtml.includes(PERFORMANCE_TRAINING_URL)||!trainingHtml.includes('Open DA Performance Insights')||!trainingHtml.includes('blocks embedded pages')||trainingHtml.includes('<iframe'))throw new Error('Performance tab must provide the large secure training-dashboard launcher without a blocked iframe');
  ...
  rivianFleet.push({name:'Van 33',vin:'1FTYR3XM2KKB78356',operational:'Operational',active:'Active',source:'Amazon fleet list',hasActive:true,hasOperational:true});
  ...
  if (morningDisplayRows(morningSections(filteredMorningRows())[0]).length !== 15) throw new Error('15-route Wave 1 template padding missing');
  ...
  if (!capturedFormattedMorning || capturedFormattedMorning.name !== 'LLOL-opening-operations-formatted.xls' || capturedFormattedMorning.type !== 'application/vnd.ms-excel' || !capturedFormattedMorning.data.includes('rowspan="15"') || !capturedFormattedMorning.data.includes('rowspan="16"') || !capturedFormattedMorning.data.includes('WAVE 1') || !capturedFormattedMorning.data.includes('11:15 (1)') || !capturedFormattedMorning.data.includes('class="spacer"') || !capturedFormattedMorning.data.includes('PRE DVIC') || !capturedFormattedMorning.data.includes('CLOCK OUT TIME') || !capturedFormattedMorning.data.includes('<x:FreezePanes/>') || !capturedFormattedMorning.data.includes('<x:SplitHorizontal>1</x:SplitHorizontal>') || !capturedFormattedMorning.data.includes('<x:Name>Morning Operations</x:Name>')) throw new Error('Formatted morning XLS export missing the exact 15-route Wave 1 layout');
