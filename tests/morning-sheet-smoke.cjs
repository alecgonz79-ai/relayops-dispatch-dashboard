...
const checks = `
  ...
  const paycomText=['Sun','7/5','JOSE LOPEZ RAMIREZ','7:30 AM - 4:00 PM','First Opening Dispatcher','Sun','7/5','STEVEN SCICCHITANO','8:00 AM - 4:30 PM','Fleet Coordinator/Resuce','Sun','7/5','VANESSA BALDERAMA','10:30 AM - 2:30 PM','Rescue','Sun','7/5','FREDY GUERRA','12:00 PM - 8:30 PM','Midshift','Sun','7/5','GERARDO GODINEZ','2:00 PM - 10:30 PM','Closing Dispatcher','Sun','7/5','WAREHOUSE PERSON','9:00 AM - 5:00 PM','Warehouse Support'].join('\\n');
  ...
  if(state.scheduleEntries.length!==6||state.scheduleEntries[1].role!=='Fleet Coordinator/Rescue'||scheduleRoleGroup(state.scheduleEntries[0].role)!=='dispatch'||scheduleRoleGroup(state.scheduleEntries[2].role)!=='driver'||scheduleRoleGroup(state.scheduleEntries[5].role)!=='other') throw new Error('Paycom schedule text classification failed');
  ...
  if(!filteredRoster.includes('Fredy Guerra')||!filteredRoster.includes('1 shown · 2 total')||!filteredRoster.includes('data-schedule-filter')) throw new Error('Paycom shift filter did not isolate matching shifts');
  ...
  if(state.scheduleDriverMarks[scheduleDriverMarkKey('Fredy Guerra')]!=='adhoc'||!state.morningRoutes.some(row=>row.driver==='Fredy Guerra'&&row.wave==='Ad hoc')||!openingRosterScheduleHtml().includes('Adhoc added')) throw new Error('Paycom Adhoc driver did not flow into Morning Sheet');
  ...
  state.morningRoutes=savedQueueMorning;state.scheduleEntries=savedScheduleEntries;state.morningOperationDate=savedOperationDate;state.messageQueueStatus={};state.callOffDriverKeys={};state.scheduleDriverMarks={};state.scheduleFilter='all';state.modal=null;state.pendingDriverText=null;state.pendingRosterSwap=null;
...
const fleetCss = fs.readFileSync('styles.css','utf8');
  ...
  if (!fleetCss.includes('body .morning-board .exact-ops-sheet .sheet-spacer-col { width:18px!important; min-width:18px!important;') || !fleetCss.includes('.exact-ops-sheet th:nth-child(10),body .morning-board .exact-ops-sheet th:nth-child(15)')) throw new Error('Morning template spacer columns I/N should stay slim');
  ...
  if (!fleetCss.includes('.exact-ops-sheet th:nth-child(22),body .morning-board .exact-ops-sheet th:nth-child(23) { width:96px; background:#92f4fa; }') || !fleetCss.includes('.exact-ops-sheet .planned-rts-cell { background:#b4a7d6; }')) throw new Error('Planned RTS header must stay cyan while its data column stays purple');
