...
const checks = `
  ...
  const sharedRosterStatus=sharedWorkspaceState(),persistentRosterStatus=persistentWorkspaceState(),reductionRoster=openingRosterScheduleHtml();if(!state.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||!sharedRosterStatus.scheduleReductions[scheduleDriverMarkKey('Casey Nolan')]||!sharedRosterStatus.scheduleStayHomeHistory['2026-06-28|casey nolan']||!persistentRosterStatus.scheduleStayHomeHistory['2026-06-28|casey nolan']||!reductionRoster.includes('Reductions')||reductionRoster.includes('data-driver-name="Casey Nolan" data-driver-role="Modified Duty">Move to backup drivers')||!reductionRoster.includes('Open Paycom login')||!reductionRoster.includes('VTO 2 · 1')||!reductionRoster.includes('VTO 4 · 0')) throw new Error('Modified Duty reduction safety, Paycom shortcut, backup totals, or shared/persistent fairness history are missing');
  ...
  if(!sharedWorkspaceState().driverNameAliases['helper person']||!persistentWorkspaceState().driverNameAliases['helper person'])throw new Error('Driver/helper aliases must sync universally across dispatchers');
