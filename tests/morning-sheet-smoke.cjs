...
const checks = `
  ...
  state.fleetExpectedCount = 0;
  refreshFleetStatus();
  if (!state.fleetRefreshPreview || state.fleetRefreshPreview.blockers.length || !modal().includes('SAFE') || !modal().includes('What changed?') || !modal().includes('No blocker found') || !modal().includes('No roster size change') || !modal().includes('Approval summary') || !modal().includes('Ready to approve after review') || !modal().includes('Safe to approve') || !modal().includes('Approve refresh') || modal().includes('Approve anyway')) throw new Error('Clean fleet refresh should show normal approval button');
...
const fleetCss = fs.readFileSync('styles.css','utf8');
  ...
  if (!fleetCss.includes('minmax(74px,1fr)') || !fleetCss.includes('min-height:42px') || !fleetCss.includes('width:19px; height:10px') || !fleetCss.includes('repeat(7,minmax(0,1fr))') || !fleetCss.includes('repeat(4,minmax(0,1fr))') || !fleetCss.includes('.fleet-card-cue') || !fleetCss.includes('.fleet-source-badges') || !fleetCss.includes('.fleet-source-badges i.ok') || !fleetCss.includes('.fleet-source-map') || !fleetCss.includes('.fleet-row-count-check') || !fleetCss.includes('.fleet-row-count-grid') || !fleetCss.includes('.fleet-name-lock-strip') || !fleetCss.includes('.fleet-name-lock-preview') || !fleetCss.includes('.fleet-source-timestamps') || !fleetCss.includes('.fleet-refresh-rule-mini') || !fleetCss.includes('.refresh-freshness-summary') || !fleetCss.includes('.fleet-refresh-what-changed') || !fleetCss.includes('.rivian-id-summary') || !fleetCss.includes('.fleet-gap-fix-tips') || !fleetCss.includes('.fleet-refresh-blockers') || !fleetCss.includes('.fleet-refresh-unknown-short') || !fleetCss.includes('.fleet-refresh-missing-vins') || !fleetCss.includes('.fleet-refresh-conflicts') || !fleetCss.includes('.fleet-refresh-missing-vins > .btn') || !fleetCss.includes('.fleet-full-export-check') || !fleetCss.includes('.fleet-refresh-guide') || !fleetCss.includes('.name-source-pill em') || !fleetCss.includes('.gap-priority') || !fleetCss.includes('.change-source-pills') || !fleetCss.includes('.warn-approve')) throw new Error('Tiny EV grid should stay compact and scan-friendly');
