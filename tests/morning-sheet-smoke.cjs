...
const fleetCss = fs.readFileSync('styles.css','utf8');
  ...
  if (!fleetCss.includes('.exact-ops-sheet th:nth-child(22) { width:67px; background:#92f4fa; }') || !fleetCss.includes('.exact-ops-sheet .planned-rts-cell { background:#b4a7d6; }')) throw new Error('Planned RTS header must use the exact reference width/cyan while its data column stays purple');
