...
const checks = `
  ...
  if(!parkingHtml.includes('data-parking-notes')||!parkingHtml.includes('Close lane after charging')||!parkingHtml.includes('data-parking-date')||!parkingHtml.includes('charger-pair')||!parkingHtml.includes('middle-1-left')||!parkingHtml.includes('parking-tent-square')||!parkingHtml.includes('stroke="currentColor"')||!parkingHtml.includes('upper-n1')||parkingHtml.includes('DRIVE LANE')) throw new Error('Editable parking notes, date, aligned split chargers, and white tent square missing');
  ...
  if(state.scheduleDriverMarks[scheduleDriverMarkKey('Vanessa Balderama')]!=='backup'||!openingRosterScheduleHtml().includes('VTO 2')||!openingRosterScheduleHtml().includes('backup-marked')) throw new Error('Rescue backup neon VTO 2 state missing');
