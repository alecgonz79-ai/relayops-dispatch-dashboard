...
vm.runInContext(`
  ...
  // This scenario imports the same shifts into each workflow explicitly.
  // Opening's call-off state stays separate from Rostering's PAYCOM source.
  storeRosteringScheduleEntries(state.scheduleEntries.map(entry=>({...entry})),'helper-box-paycom.xls');
  ...
  storeRosteringScheduleEntries(state.scheduleEntries.map(entry=>({...entry})),'route-cap-paycom.xls');
  ...
  state.fitOpeningPicklistRows=true;state.openingPicklistWaveSlots=6;state.openingPicklistShowAdhoc=true;state.morningRoutes=[['CX1','11:15 AM','1'],['CX2','11:20 AM','1'],['CX3','11:25 AM',''],['CX4','11:40 AM',''],['CX5','11:45 AM',''],['CX6','11:50 AM',''],['AX','Ad hoc','']].map(([route,wave,ev],index)=>({routeUid:'u'+index,dsp:'LLOL',driver:'Driver '+index,route,wave,staging:'STG.V.'+(index+1),pad:'A',service:route==='AX'?'Adhoc':'Standard Parcel',ev,deviceName:'',portable:'',stops:100,packages:200}));
  const sections=openingPicklistSections(),duplicateSection={...sections[0],rows:state.morningRoutes.slice(0,2),capacity:2,hasTime:true};globalThis.__picklist={waveCaps:sections.slice(0,6).map(row=>row.capacity),adhoc:sections.find(row=>row.key==='adhoc')?.capacity,duplicate:openingPicklistSectionHtml(duplicateSection,0),morning:morningWaveGroup(duplicateSection,0)};
