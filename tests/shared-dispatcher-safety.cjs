...
vm.runInContext(`
  ...
  rivianFleet.splice(0,rivianFleet.length,{name:'EV1',vin:'7FCEHEB79PN000001',battery:96,miles:150,operational:'Operational',active:'Active',vehicleType:'Rivian EDV 700',source:'Amazon fleet list + FleetOS tracker',hasBattery:true,hasActive:true,hasOperational:true},{name:'EV2',vin:'7FCEHEB79PN000002',battery:97,miles:151,operational:'Operational',active:'Active',vehicleType:'Rivian EDV 700',source:'Amazon fleet list + FleetOS tracker',hasBattery:true,hasActive:true,hasOperational:true});
  state.equipmentImport={details:{'1':{device:'D1',portable:'P1'},'2':{device:'D2',portable:''}}};globalThis.__readyPool=automaticFleetVehiclePool({electricOnly:true}).map(row=>row.label);
...
assert(context.__readyPool.join(',')==='EV1',`Automatic assignment must exclude a vehicle missing either Device or Portable (got ${context.__readyPool.join(',')||'empty'})`);
