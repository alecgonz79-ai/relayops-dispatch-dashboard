...
assert(app.includes("UI_SCROLL_MEMORY_SELECTORS=['.sheet-scroll','.opening-picklist-scroll','.device-sheet-table-wrap','.parking-lot']") && app.includes('restoreUiScrollMemory(scrollMemory)'), 'Morning Sheet, Picklist, Device, and Parking panes must preserve their own scroll positions across renders');
...
assert(app.includes('data-screenshot-review-pad=') && app.includes('function saveScreenshotReviewPad(input,commit=false)') && app.includes("setMorningPadForSection('',wave,value)") && app.includes('morningSectionPadOverrides'), 'Pad letters must be directly editable from the pre-screenshot review and update shared Morning Sheet rows');
...
assert(app.includes('class="opening-picklist-swap-rail"') && /openingPicklistRightHtml\([^)]*\)}<aside class="opening-picklist-swap-rail"/.test(app), 'The Cortex swap tracker must render in its own third column to the right of the Picklist operations boxes');
