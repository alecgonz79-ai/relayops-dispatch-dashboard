...
if (sheet.getMaxRows() !== 125) throw new Error(`Connector should expand the dated template to 125 rows, got ${sheet.getMaxRows()}`);
...
if (sheet.getCell(18, 1) !== '11:15 (2)' || sheet.getCell(35, 1) !== '11:20 (0)' || sheet.getCell(52, 1) !== '11:25 (0)' || sheet.getCell(69, 1) !== '11:40 (0)' || sheet.getCell(86, 1) !== '11:45 (0)') throw new Error('Connector should write all five wave time/count labels after 15 route rows');
if (sheet.getCell(20, 1) !== 'WAVE 2' || sheet.getCell(37, 1) !== 'WAVE 3' || sheet.getCell(88, 1) !== "ADHOC's" || sheet.getCell(120, 1) !== 'DSP') throw new Error('Connector should preserve expanded OPS LOG 2026 section anchors');
...
if (resultContext.__result.writeRange !== 'A3:V' || resultContext.__result.writtenRange !== 'A3:V125' || resultContext.__result.lastCell !== 'V125') throw new Error('Connector should return the expanded A3:V125 template proof');
...
if (wave5Sheet.getCell(86, 1) !== '11:45 (15)' || wave5Sheet.getCell(88, 1) !== "ADHOC's") throw new Error('Expanded Wave 5 should keep its footer at row 86 and preserve the Adhocs anchor at row 88');
...
for (let waveIndex = 0; waveIndex < 5; waveIndex += 1) {
  ...
  for (let routeIndex = 0; routeIndex < 15; routeIndex += 1) {
    capacityRows.push([
      ...
      routeIndex === 0 ? ['A','B','C','A','B'][waveIndex] : '',
      String(waveIndex * 15 + routeIndex + 1),
      `D${routeIndex + 1}`,
  ...
  capacityRows.push([`11:${15 + waveIndex * 5} (15)`, '', '', '', '', '', '', '', '', '', '', '', '']);
  ...
  capacitySections.push({
    ...
    wave:`11:${15 + waveIndex * 5} AM`,
    driverCount:15,
    pad:['A','B','C','A','B'][waveIndex],
    sourceIndex,
    startRow:3 + waveIndex * 17,
    rowCount:15,
    timeRow:18 + waveIndex * 17,
    separatorRow:19 + waveIndex * 17
  });
}
const adhocSourceIndex = capacityRows.length;
for (let routeIndex = 0; routeIndex < 15; routeIndex += 1) {
  capacityRows.push(['', `Adhoc Driver ${routeIndex + 1}`, `AX${routeIndex + 1}`, '', '', '', '', '', '', '', '', '', '']);
  capacityRowTypes.push('route');
}
capacityRows.push(Array(13).fill(''));
capacityRowTypes.push('separator');
capacitySections.push({label:"ADHOC's",sourceIndex:adhocSourceIndex,startRow:88,rowCount:15,separatorRow:103});
...
if (!fullCapacityContext.__validation.ready) throw new Error(`15-row Wave/Adhoc preflight should pass: ${fullCapacityContext.__validation.errors.join('; ')}`);
if (fullCapacitySheet.getCell(17, 2) !== 'Wave 1 Driver 15' || fullCapacitySheet.getCell(85, 2) !== 'Wave 5 Driver 15' || fullCapacitySheet.getCell(102, 2) !== 'Adhoc Driver 15') throw new Error('Connector should write 15 rows in Waves 1-5 and Adhocs without overlap');
if (String(fullCapacitySheet.getCell(19, 1) || '') !== '' || fullCapacitySheet.getCell(20, 1) !== 'WAVE 2' || String(fullCapacitySheet.getCell(87, 1) || '') !== '' || fullCapacitySheet.getCell(88, 1) !== "ADHOC's") throw new Error('15-row sections should retain black divider rows and downstream anchors');
...
if (rtsSheet.getCell(3, 21) !== '8:45 PM' || rtsSheet.getCell(4, 21) !== '9:06 PM' || rtsSheet.getCell(18, 1) !== '11:15 (2)' || rtsSheet.getCell(69, 1) !== '11:40 (0)' || rtsSheet.getCell(86, 1) !== '11:45 (0)') throw new Error('RTS-only connector should write only Planned RTS and all wave-time/count cells');
...
if (creationContext.__result.sheetName !== '7/12/26' || !creationContext.__result.createdSheet || !createdDateSheet || createdDateSheet.getCell(3, 2) !== 'Driver One' || createdDateSheet.getCell(18, 1) !== '11:15 (2)') throw new Error('Connector should create the exact operation-date tab from OPS LOG 2026 and populate expanded anchors');
