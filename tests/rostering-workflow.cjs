...
function run() {
  ...
  vm.runInContext(`
    ...
    mayaProfile.customFlags=['Lives far away'];globalThis.__customFlagSummary=driverFlagSummary('Maya Collins');globalThis.__normalizedCustomFlags=normalizeDriverProfiles(state.driverProfiles)[driverProfileEntry('Maya Collins').key].customFlags;
    state.morningOperationDate='2026-07-15';state.scheduleBackupRecords={'2026-07-15|maya collins':{name:'Maya Collins',role:'Delivery Associate',vto:'VTO 4'}};state.scheduleDriverMarks={};state.earlyCalloffAcknowledgements={};globalThis.__earlyReminderRows=earlyCalloffReminderRows();state.modal='early-calloff-reminder';globalThis.__earlyReminderModal=modal();state.modal=null;acknowledgeEarlyCalloffReminder();globalThis.__earlyReminderAfterAck=earlyCalloffReminderRows();
    state.lastMorningImportFingerprint='2026-07-15|test-import';state.padCheckAcknowledgements={};state.modal='pad-check-reminder';globalThis.__padReminderModal=modal();state.modal=null;acknowledgePadCheckReminder();globalThis.__padAck=Boolean(state.padCheckAcknowledgements['2026-07-15|test-import']);
    globalThis.__nav={sections:NAV.map(group=>group.section),achatSection:NAV.find(group=>group.items.some(item=>item[0]==='achat'))?.section,achatIndex:NAV.find(group=>group.items.some(item=>item[0]==='achat'))?.items.findIndex(item=>item[0]==='achat'),adminIndex:NAV.find(group=>group.items.some(item=>item[0]==='admin'))?.items.findIndex(item=>item[0]==='admin')};
  ...
  assert(context.__autoResult.drivers === 3 && context.__autoResult.helpers === 1 && context.__autoResult.mode === 'abc' && context.__autoRows.join(',') === 'Evan Stone,Maya Collins,Nina Patel', 'ABC Auto Roster must place Delivery Associates plus only fairness-flagged Rescue shifts alphabetically');
  assert(context.__eligibility.delivery&&context.__eligibility.flaggedRescue&&!context.__eligibility.plainRescue&&!context.__eligibility.fleet&&context.__eligibility.fleetCategory==='other'&&context.__eligibility.fleetGroup==='dispatch', 'Auto Roster eligibility must exclude normal VTO 2 and Fleet Coordinator shifts while allowing fairness-flagged Rescue drivers');
  ...
  assert(context.__orders.abc.join(',') === 'Alpha,Bravo,Charlie' && context.__orders.random.join(',') === 'Bravo,Charlie,Alpha' && context.__orders.randomFallback.join(',')!=='Alpha,Bravo,Charlie', 'Auto Roster must offer deterministic ABC order and a random mode that can never fall back to alphabetical order');
  assert(context.__renderedRandomOrder.join(',')==='Bravo,Charlie,Alpha', 'Roster rendering must preserve shuffled assignment order instead of sorting equal-time drivers alphabetically');
  ...
  assert(!Object.values(context.__backupGroups).flat().some(row=>/Parker Pilot|Moe Modified/.test(row.name)), 'Pilot/Rescues and Modified Duty shifts must stay out of every unrostered backup group');
  assert(context.__specialRoles.every(row=>row.category==='other'&&row.group==='other'&&!row.eligible&&!row.helper), 'Pilot/Rescues and Modified Duty shifts must be non-rosterable Other roles everywhere');
  assert(context.__paycomHtml.includes('Parker Pilot')&&context.__paycomHtml.includes('Moe Modified')&&(context.__paycomHtml.match(/>Other role<\/button>/g)||[]).length>=2, 'Non-rosterable operational support shifts must remain visible in PAYCOM with disabled Other role actions');
  ...
  assert(context.__emailTemplate.includes('Helpers:\n\nJohn Helper\n\nRide Alongs:\n\nRiley R\n\n---------------------------------------------------\n\nBack Ups:\n\nVTO (2)'), 'Helpers, Ride Alongs, separator, Back Ups, and VTO 2 must preserve the requested blank-line layout');
  ...
  assert(context.__emailTemplateHtml.includes('<strong>Opener:</strong>')&&context.__emailTemplateHtml.includes('<strong>MidShift:</strong>')&&context.__emailTemplateHtml.includes('<strong>Ride Alongs:</strong>')&&context.__emailTemplateHtml.includes('<strong><u>VTO (2)</u></strong>')&&context.__emailTemplateHtml.includes('<strong><u>VTO (4)</u></strong>'), 'Rich email output must keep the Ride Alongs heading bold and VTO headings bold and underlined');
  ...
  assert(context.__screenshotLockedImport.date==='2026-07-16'&&context.__screenshotLockedImport.lockedDate==='2026-07-16'&&context.__screenshotLockedImport.services.join(',')==='rivian-medium'&&context.__screenshotLockedImport.importName==='one-block.png'&&context.__screenshotLockedImport.paycomImportName==='paycom-next-day.xls'&&context.__screenshotLockedImport.entries.includes('Screenshot Driver One')&&!context.__screenshotLockedImport.nextDayPlan, 'Rostering PAYCOM imports must not switch away from or add default blocks to an active screenshot-created roster');
  ...
  assert(context.__flagLabels.length===9&&['Late Driver','Violations','Performance','Suspended','Modified Duty','Only Later Waves','Likes Helper Routes','New Driver','Call off before 9:10 AM'].every(label=>context.__flagLabels.includes(label)), 'PAYCOM Driver Notes must preserve all nine supported flags, including the early call-off safety flag');
  ...
  assert((context.__flagsModal.match(/data-driver-flag-option/g)||[]).length===9&&context.__flagsModal.includes('Call off before 9:10 AM')&&context.__flagsModal.includes('Type a custom flag'), 'Driver Notes editor must render all nine profile flags and custom flag controls');
  assert(context.__customFlagSummary.includes('Lives far away')&&context.__normalizedCustomFlags.includes('Lives far away'), 'Custom driver flags must render and survive shared profile normalization');
  assert(context.__earlyReminderRows.length===1&&context.__earlyReminderRows[0].name==='Maya Collins'&&context.__earlyReminderModal.includes('Call these drivers before 9:10 AM')&&context.__earlyReminderModal.includes('Required acknowledgement')&&context.__earlyReminderAfterAck.length===0, 'Flagged VTO/stay-home drivers must trigger one required early call-off reminder and stay dismissed after acknowledgement');
  assert(context.__padReminderModal.includes('Enter today’s pads')&&context.__padReminderModal.includes('PAD values are always manual')&&(context.__padReminderModal.match(/WAVE [1-6]/g)||[]).length===6&&context.__padAck, 'Morning imports must show all six manual pad checks and save the required acknowledgement');
  ...
  assert(context.__nav.sections.join('|')==='Opening Operations|Closing Operations|Improve'&&context.__nav.achatSection==='Improve'&&context.__nav.achatIndex===3&&context.__nav.adminIndex===4, 'Admin access must sit directly below A-Chat inside Improve');
  assert(!context.__daily.rosteringPlans && context.__persistent.rosteringPlans && !context.__daily.rosteringTrainingMatches && context.__persistent.rosteringTrainingMatches, 'Rostering plans and training matches must sync once in station-persistent state');
