...
assert(app.includes('readonly aria-readonly="true"')&&app.includes('title="Double-click to edit')&&app.includes('tap once on mobile')&&app.includes('parkingTapOpensEditor(event)')&&app.includes('beginParkingSlotEdit(el)'),'Fleet parking van fields must support the same one-tap mobile and double-click desktop editing as the dispatcher map');
