...
if(status.expected.length!==2||status.preComplete.length!==2||status.postComplete.length!==1||!status.missingPost.some(row=>row.name==='Maya Collins')||status.expected.some(row=>row.name==='Fredy Guerra'))throw new Error('Morning Sheet/Picklist-only Whiparound comparison failed');
const appliedChecks=applyWhiparoundChecksToMorning('2026-07-12');
if(appliedChecks.routes!==2||state.morningRoutes[0].preWhip!==true||state.morningRoutes[0].postWhip!==false||state.morningRoutes[1].preWhip!==true||state.morningRoutes[1].postWhip!==true)throw new Error('Whiparound import did not check PRE-WHIP/POST-WHIP on the Morning Sheet');
const html=inboxPage();
for(const token of ['Whiparound Import','CSV / XLSX','Send Whiparound Checks','Morning Sheet / Picklist only','Missing Pre-Trip DVIR','Missing Post-Trip DVIR','Text to remind driver','Not on Route','Prewritten reminders','Completed inspections'])if(!html.includes(token))throw new Error('Whiparound UI missing '+token);
markWhiparoundNotOnRoute('Maya Collins');
if(whiparoundStatus('2026-07-12').expected.some(row=>row.name==='Maya Collins'))throw new Error('Not on Route removal failed');
restoreWhiparoundDriver('maya collins');
if(!whiparoundStatus('2026-07-12').expected.some(row=>row.name==='Maya Collins'))throw new Error('Whiparound restore failed');
