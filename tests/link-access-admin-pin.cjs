...
(async()=>{
  ...
  const activeQueue='relayops_cloud_queue:station-1:2026-07-20',priorQueue='relayops_cloud_queue:station-1:2026-07-19',otherStationQueue='relayops_cloud_queue:station-2:2026-07-20';
  storageMap.set(activeQueue,JSON.stringify({shared:true,userId:session.user.id,payload:{morningRoutes:[{route:'CX-QUEUED'}]},persistentPayload:{},basePayload:{},basePersistentPayload:{}}));
  storageMap.set(priorQueue,'prior date must remain');storageMap.set(otherStationQueue,'other station must remain');storageMap.set('relayops_sheet_history','oversized undo cache');
  assert(cloud.reclaimStorageForSharedSession()>0,'Storage-quota recovery did not reclaim redundant cache space');
  assert(!storageMap.has(activeQueue)&&!storageMap.has('relayops_sheet_history'),'Quota recovery must move only the active queue to memory and clear redundant caches');
  assert(storageMap.get(priorQueue)==='prior date must remain'&&storageMap.get(otherStationQueue)==='other station must remain','Quota recovery changed another date or station queue');
  storageMap.delete(priorQueue);
  assert(cloud.canSafelySwitch()===false,'The active queue was discarded instead of remaining recoverable in memory');
