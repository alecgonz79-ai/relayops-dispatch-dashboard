...
(async()=>{
  ...
  // Another date's queue must block navigation, not be silently deleted/flushed
  // into the current date or the other station.
  const oldQueue=`relayops_cloud_queue:${DUR6}:2026-09-06`;sharedBrowserStorage.setItem(oldQueue,JSON.stringify({shared:true,userId:'dispatcher-2',payload:{morningRoutes:[{id:'old'}]},persistentPayload:{}}));assert.equal((await popup.cloud.prepareStationSwitch()).ok,false);assert(sharedBrowserStorage.getItem(oldQueue));sharedBrowserStorage.removeItem(oldQueue);
  sharedBrowserStorage.setItem(oldQueue,'corrupt queue preserved');assert.equal((await popup.cloud.prepareStationSwitch()).ok,false);assert.equal(sharedBrowserStorage.getItem(oldQueue),'corrupt queue preserved');sharedBrowserStorage.removeItem(oldQueue);
  ...
  for(const page of [djt,dur,fixed,home,popup,failing,conflict,concurrent])page.close();
