...
const checks = `
  ...
  if (!connectorHtml.includes('GOOGLE SHEETS CONNECTOR') || !connectorHtml.includes('Copy Apps Script') || !connectorHtml.includes('Copy setup checklist') || !connectorHtml.includes('Download .gs file') || !connectorHtml.includes('RelayOps menu') || !connectorHtml.includes('Preflight ready') || !connectorHtml.includes('Handoff proof') || !connectorHtml.includes('Connector rows') || !connectorHtml.includes('A–M write scope only') || !connectorHtml.includes('Columns N+ stay untouched') || !connectorHtml.includes('writes A3:M only') || !connectorHtml.includes('Every row has 13 columns') || !connectorHtml.includes(MORNING_APPS_SCRIPT_URL) || !connectorHtml.includes('Test connector') || !connectorHtml.includes('Dry run') || !connectorHtml.includes('Send to Google Sheet') || !connectorHtml.includes('Connector payload preview')) throw new Error('Morning Sheets connector modal missing');
