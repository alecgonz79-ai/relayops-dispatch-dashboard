...
const checks = `
  ...
  const expectedSectionStarts=[3,18,33,48,63,79,95,111],expectedSectionCapacities=[13,13,13,13,14,14,15,15],expectedSectionTimes=[16,31,46,61,77,93,109,125],expectedSectionSeparators=[17,32,47,62,78,94,110,126];
  if(payload.sections.length!==8||payload.sections.some((section,index)=>section.startRow!==expectedSectionStarts[index]||section.rowCount!==expectedSectionCapacities[index]||section.timeRow!==expectedSectionTimes[index]||section.separatorRow!==expectedSectionSeparators[index])) throw new Error('Morning Sheets payload must match the exact six-wave 132-row source geometry through Helpers');
  ...
  if (!adhocSection || adhocSection.hasTimeRow !== false || adhocSection.startRow !== 95 || adhocSection.timeRow !== 109 || adhocSection.separatorRow !== 110 || !helperSection || helperSection.hasTimeRow !== false || helperSection.startRow !== 111 || helperSection.timeRow !== 125 || helperSection.separatorRow !== 126) throw new Error('ADHOC and HELPERS must retain the exact no-time-row layout after six fixed-capacity Wave sections');
  ...
  if (!script.includes('function relayOpsTemplateLayout') || !script.includes('const neededRows = RELAYOPS_LAST_ROW') || !script.includes('hasEnoughRows: sheet.getMaxRows() >= neededRows') || !script.includes('hasEnoughColumns: sheet.getMaxColumns() >= RELAYOPS_TEMPLATE_COLS')) throw new Error('Apps Script should verify the fixed 132-row Google Sheet template capacity before writing');
  if (script.includes('insertRowsAfter') || script.includes('insertColumnsAfter')) throw new Error('Fixed 132-row connector must not mutate the authoritative template geometry');
  if (!script.includes('const result = writeRelayOpsMorningSheet(payload)') || !script.includes('sheet: result.sheetName') || !script.includes('startCell: result.startCell') || !script.includes('writeRange: result.writeRange') || !script.includes('writtenRange: result.writtenRange') || !script.includes("writtenRange: 'A3:V132'") || !script.includes("lastCell: 'V132'")) throw new Error('Apps Script confirmation should report the exact A3:V132 range');
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || MORNING_CONNECTOR_BUILD !== '2026-07-29-six-wave-132-row-layout' || !testMorningSheetsConnector.toString().includes('MORNING_CONNECTOR_BUILD') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify the current six-wave 132-row deployment');
