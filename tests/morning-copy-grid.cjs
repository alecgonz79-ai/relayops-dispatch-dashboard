...
const checks = `
  ...
  if (!clipHtml.includes('<colgroup>') || !clipHtml.includes('width:210px') || !clipHtml.includes('width:18px')) throw new Error('Clipboard HTML should include fixed A-V column width hints');
  ...
  if (!fallbackHtml.includes('<colgroup>') || !fallbackHtml.includes('width:210px') || !fallbackHtml.includes('width:18px')) throw new Error('TSV HTML fallback should include fixed A-V column width hints');
  ...
  if (!setupHtml.includes('width:210px') || setupHtml.includes('width:18px') || !setupHtml.includes('rowspan=') || !setupHtml.includes('WAVE 1')) throw new Error('A-H wave setup copy should preserve merged Wave/Pad cells');
  if (!countsHtml.includes('width:86px') || countsHtml.includes('width:210px') || countsHtml.includes('rowspan=')) throw new Error('P-Q counts copy should contain count widths only');
  if (!rtsHtml.includes('width:96px') || rtsHtml.includes('width:210px') || rtsHtml.includes('rowspan=')) throw new Error('U planned RTS copy should contain planned RTS only');
