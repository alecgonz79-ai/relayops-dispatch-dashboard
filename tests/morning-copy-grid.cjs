...
const checks = `
  ...
  if (!html.includes('Filtered waves') || !html.includes('Fixed Ops Log check') || !html.includes('Dated Ops Log') || !html.includes('Connect Google Sheet') || !html.includes('Dry run happens before every send') || !html.includes('Copy fallback') || !html.includes('Handoff proof') || !html.includes('Google range') || !html.includes('Connector rows') || !html.includes('Ready for copy fallback') || !html.includes('Import / route source') || !html.includes('Visible rows = payload') || !html.includes('Exact connector')) throw new Error('Fixed OPS LOG 2026 handoff UI should show date-only Google bridge and row proof');
  ...
  if (!connectorHtml.includes('GOOGLE SHEETS CONNECTOR') || !connectorHtml.includes('COPY REVISED APPS SCRIPT') || !connectorHtml.includes('Legacy Logistics Operations') || !connectorHtml.includes('OPS LOG 2026') || !connectorHtml.includes('exact date only') || !connectorHtml.includes('Wave 1 row 3') || !connectorHtml.includes('DSP row 110') || !connectorHtml.includes('Copy setup checklist') || !connectorHtml.includes('Download .gs file') || !connectorHtml.includes(MORNING_APPS_SCRIPT_URL) || !connectorHtml.includes('Test connector') || !connectorHtml.includes('Dry run') || !connectorHtml.includes('Send to Google Sheet')) throw new Error('Fixed OPS LOG 2026 connector modal is missing date-only mapping/setup guidance');
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || !testMorningSheetsConnector.toString().includes('2026-07-11-main-ops-log-fixed-layout') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify OPS LOG 2026 fixed-layout deployment');
