...
const checks = `
  ...
  if (!script.includes('function onOpen') || !script.includes("createMenu('RelayOps')") || !script.includes('relayOpsConnectorStatus') || !script.includes('Run demo write')) throw new Error('Apps Script should add a RelayOps menu for install verification');
  ...
  if (!script.includes('function validateRelayOpsMorningPayload') || !script.includes('Header row must match A-M template') || !script.includes('Row types must match row count') || !script.includes('Separator row ') || !script.includes('has invalid merge rows')) throw new Error('Apps Script should validate payload before writing');
  if (!script.includes('payload.dryRun') || !script.includes('dryRun: true') || !script.includes('function findRelayOpsMorningSheet')) throw new Error('Apps Script should support dry-run validation without writing cells');
  ...
  if (!connectorHtml.includes('GOOGLE SHEETS CONNECTOR') || !connectorHtml.includes('Copy Apps Script') || !connectorHtml.includes('Copy setup checklist') || !connectorHtml.includes('Download .gs file') || !connectorHtml.includes('RelayOps menu') || !connectorHtml.includes('Preflight ready') || !connectorHtml.includes('Every row has 13 columns') || !connectorHtml.includes(MORNING_APPS_SCRIPT_URL) || !connectorHtml.includes('Test connector') || !connectorHtml.includes('Dry run') || !connectorHtml.includes('Send to Google Sheet') || !connectorHtml.includes('Connector payload preview')) throw new Error('Morning Sheets connector modal missing');
  ...
  if (!dryRunMorningToSheets.toString().includes('dryRun:true') || !dryRunMorningToSheets.toString().includes('Connector did not confirm dry run mode') || !dryRunMorningToSheets.toString().includes('state.morningSheetsLastDryRun')) throw new Error('Dry run should validate through Apps Script without writing cells');
  const setupChecklist = morningSheetsSetupChecklist();
  if (!setupChecklist.includes(MORNING_TEMPLATE_URL) || !setupChecklist.includes('Extensions > Apps Script') || !setupChecklist.includes('Deploy > New deployment > Web app') || !setupChecklist.includes('Execute as: Me') || !setupChecklist.includes('Anyone with the link') || !setupChecklist.includes('Dry run') || !setupChecklist.includes('Last confirmed Google write')) throw new Error('Morning Sheets setup checklist missing required install steps');
  ...
  state.morningSheetsLastReceipt = {sheet:'Morning Operations',startCell:'A3',rows:44,sections:5,updatedAt:'2026-07-08T12:00:00.000Z'};
  const receiptHtml = morningSheetsReceiptHtml();
  if (!receiptHtml.includes('Last confirmed Google write') || !receiptHtml.includes('Morning Operations') || !receiptHtml.includes('A3') || !receiptHtml.includes('44') || !receiptHtml.includes('5')) throw new Error('Morning Sheets receipt UI missing confirmed write details');
