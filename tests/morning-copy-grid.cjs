...
const checks = `
  ...
  if (!Array.isArray(payload.waves) || payload.waves.length !== 5 || payload.waves.some((wave,index)=>morningFixedSectionKey(wave.label)!==('WAVE'+(index+1))||!String(wave.value||'').trim())) throw new Error('Morning Sheets connector must always include Wave 1-5 time/count labels');
  ...
  if (!adhocSection || adhocSection.hasTimeRow !== false || adhocSection.timeRow !== 102 || adhocSection.separatorRow !== 103 || !helperSection || helperSection.hasTimeRow !== false || helperSection.timeRow !== 118 || helperSection.separatorRow !== 119) throw new Error('ADHOC and HELPERS must retain real no-time-row layout after five 15-route Wave sections');
  const preflight = morningSheetsPreflight(payload);
  if (!preflight.ready || preflight.checks.length !== 8 || !preflight.checks.every(check => check.ok)) throw new Error('Morning Sheets preflight should pass for the current A-M payload and all five wave labels');
  const preflightHtml = morningSheetsPreflightHtml(payload);
  if (!preflightHtml.includes('Preflight ready') || !preflightHtml.includes('A–M write scope only') || !preflightHtml.includes('Columns N+ stay untouched') || !preflightHtml.includes('Every row has 13 columns') || !preflightHtml.includes('Black dividers are real rows') || !preflightHtml.includes('All five wave times ready') || !preflightHtml.includes('Wave/Pad merge map ready') || !preflightHtml.includes('Target cell A3')) throw new Error('Morning Sheets preflight UI missing required checks');
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || !testMorningSheetsConnector.toString().includes('2026-07-27-fifteen-route-wave-layout') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify the current 15-route Wave deployment');
