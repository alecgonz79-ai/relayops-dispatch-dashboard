...
const checks = `
  ...
  if (!script.includes('const result = writeRelayOpsMorningSheet(payload)') || !script.includes('sheet: result.sheetName') || !script.includes('startCell: result.startCell') || !script.includes('writeRange: result.writeRange') || !script.includes('writtenRange: result.writtenRange') || !script.includes("writtenRange: 'A3:V' + lastRow") || !script.includes("lastCell: 'V' + lastRow")) throw new Error('Apps Script confirmation should report the original A-V template range');
  ...
  if (!script.includes('RELAYOPS_TEMPLATE_COLS = 22') || !script.includes("RELAYOPS_TEMPLATE_RANGE = 'A3:V'") || !script.includes('row.slice(0, 8)') || !script.includes('return [row[9]]') || !script.includes('return [row[10]]') || !script.includes('return [row[12]]')) throw new Error('Apps Script should map compact dashboard rows into A:H, P:Q, and U');
  ...
  if (!connectorHtml.includes('GOOGLE SHEETS CONNECTOR') || !connectorHtml.includes('COPY REVISED APPS SCRIPT') || !connectorHtml.includes('original Ops Log template') || !connectorHtml.includes('A:V') || !connectorHtml.includes('A:H') || !connectorHtml.includes('P:Q') || !connectorHtml.includes('Planned RTS') || !connectorHtml.includes('Copy setup checklist') || !connectorHtml.includes('Download .gs file') || !connectorHtml.includes(MORNING_APPS_SCRIPT_URL) || !connectorHtml.includes('Test connector') || !connectorHtml.includes('Dry run') || !connectorHtml.includes('Send to Google Sheet')) throw new Error('Original Ops Log connector modal is missing revised mapping/setup guidance');
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || !testMorningSheetsConnector.toString().includes('2026-07-11-legacy-ops-log') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify the original A-V range and current deployment build');
