...
const checks = `
  ...
  const workflowRequirements=['Morning setup','Upload DJT6 day files','Assign safe vans','Add devices','Send DJT6 Morning Sheet','Later: add RTS times','Import RTS times','Send RTS times','Copy fallback'];
  ...
  if(!diagnostics.includes('Setup & diagnostics')||!diagnostics.includes('DJT6 Ops Log connector setup')||!diagnostics.includes('creates it by duplicating the station template')||!diagnostics.includes('Handoff proof')||!diagnostics.includes('Google Sheets structure proof'))throw new Error('Top diagnostics modal should retain station-scoped connector guidance and proof details');
  state.modal=null;
  ...
  state.copyMode = true;
  if (html.includes('class="wave-separator"') && html.includes('colspan="22"')) throw new Error('Separator rows must be real A-V grid cells, not colspan');
  ...
  if (sepCellCount !== 22) throw new Error('Separator row should contain 22 A-V cells, got ' + sepCellCount);
  const tsv = morningSheetTsv();
  const rows = tsv.split('\\n');
  if (rows.length < 10) throw new Error('Morning TSV too short');
  rows.forEach((row, index) => {
    const cols = row.split('\\t');
    if (cols.length !== 22) throw new Error('TSV row ' + (index + 1) + ' has ' + cols.length + ' columns instead of 22');
  });
  ...
  if (sheetCopyZone(8).join(',') !== '8,8' || sheetCopyZone(13).join(',') !== '13,13') throw new Error('Black spacer columns I/N must be real selectable boundaries');
  if (sheetCopyZone(9).join(',') !== '9,12' || sheetCopyZone(12).join(',') !== '9,12') throw new Error('Inspection block should stay between black dividers');
  if (sheetCopyZone(20).join(',') !== '14,21') throw new Error('Operations block should include planned RTS column U');
  if (typeof writeClipboardTable !== 'function') throw new Error('Rich clipboard table writer missing');
  if (JSON.stringify(morningClipboardColumnWidths) !== JSON.stringify([56,149,52,43,64,34,51,54,15,55,59,59,58,15,53,65,69,87,74,43,67,75])) throw new Error('A-V widths must match the supplied 1297px reference image exactly');
  const clipHtml = morningSheetClipboardHtml();
  if (!clipHtml.includes('<table')) throw new Error('Clipboard HTML table missing');
  if (!clipHtml.includes('<colgroup>') || !clipHtml.includes('width:149px') || !clipHtml.includes('width:15px')) throw new Error('Clipboard HTML should include exact reference A-V column width hints');
  ...
  if (!fallbackHtml.includes('<colgroup>') || !fallbackHtml.includes('width:149px') || !fallbackHtml.includes('width:15px')) throw new Error('TSV HTML fallback should include exact reference A-V column width hints');
  ...
  const countsHtml = copyRowsClipboardHtml(firstWaveItems,15,16);
  const rtsHtml = copyRowsClipboardHtml(firstWaveItems,20,20);
  if (!setupHtml.includes('width:149px') || setupHtml.includes('width:15px') || !setupHtml.includes('rowspan=') || !setupHtml.includes('WAVE 1')) throw new Error('A-H wave setup copy should preserve exact widths and merged Wave/Pad cells');
  if (!countsHtml.includes('width:65px') || countsHtml.includes('width:149px') || countsHtml.includes('rowspan=')) throw new Error('P-Q counts copy should contain exact count widths only');
  if (!rtsHtml.includes('width:67px') || rtsHtml.includes('width:149px') || rtsHtml.includes('rowspan=')) throw new Error('U planned RTS copy should contain the exact Planned RTS width only');
  ...
  if (!Array.isArray(payload.waves) || payload.waves.length !== 6 || payload.waves.some((wave,index)=>morningFixedSectionKey(wave.label)!==('WAVE'+(index+1))||!String(wave.value||'').trim())) throw new Error('Morning Sheets connector must always include Wave 1-6 time/count labels');
  ...
  const expectedSectionStarts=[3,20,37,54,71,88,105,121],expectedSectionCapacities=[15,15,15,15,15,15,15,15],expectedSectionTimes=[18,35,52,69,86,103,119,135],expectedSectionSeparators=[19,36,53,70,87,104,120,136];
  if(payload.sections.length!==8||payload.sections.some((section,index)=>section.startRow!==expectedSectionStarts[index]||section.rowCount!==expectedSectionCapacities[index]||section.timeRow!==expectedSectionTimes[index]||section.separatorRow!==expectedSectionSeparators[index])) throw new Error('Morning Sheets payload must match the exact six-wave 142-row source geometry through Helpers');
  ...
  if (!adhocSection || adhocSection.hasTimeRow !== false || adhocSection.startRow !== 105 || adhocSection.timeRow !== 119 || adhocSection.separatorRow !== 120 || !helperSection || helperSection.hasTimeRow !== false || helperSection.startRow !== 121 || helperSection.timeRow !== 135 || helperSection.separatorRow !== 136) throw new Error('ADHOC and HELPERS must retain the exact no-time-row layout after six fixed-capacity Wave sections');
  const preflight = morningSheetsPreflight(payload);
  if (!preflight.ready || preflight.checks.length < 8 || !preflight.checks.every(check => check.ok)) throw new Error('Morning Sheets preflight should pass for the current A-M payload and all six wave labels');
  const maxWaveRows=Array.from({length:15},(_,index)=>[index===0?'WAVE 1':'','Capacity Driver '+(index+1),'CX'+(800+index),'STG.V.'+(index+1),index===0?'A':'',String(index+1),'D'+(index+1),'P'+(index+1),'',String(180+index),String(300+index),'','8:45 PM']);
  const maxWavePayload={...payload,writeMode:'full-replace',rows:[...maxWaveRows,['11:15 (15)','','','','','','','','','','','',''],Array(13).fill('')],rowTypes:[...Array(15).fill('route'),'time','separator'],sections:[{label:'WAVE 1',slotKey:'WAVE1',wave:'11:15 AM',driverCount:15,pad:'A',sourceIndex:0,startRow:3,rowCount:15,timeRow:18,hasTimeRow:true,separatorRow:19,separatorRows:[19]}],waves:payload.waves.map((wave,index)=>index===0?{...wave,value:'11:15 (15)'}:wave)};
  const maxWavePreflight=morningSheetsPreflight(maxWavePayload);
  if(!maxWavePreflight.ready)throw new Error('Client preflight must accept all 15 available route rows in a Wave');
  const overflowWavePayload={...maxWavePayload,rows:[...maxWaveRows,['','Capacity Driver 16','CX815','STG.V.16','','16','D16','P16','','195','315','','8:45 PM'],['11:15 (16)','','','','','','','','','','','',''],Array(13).fill('')],rowTypes:[...Array(16).fill('route'),'time','separator'],sections:[{...maxWavePayload.sections[0],driverCount:16,rowCount:16,timeRow:19,separatorRow:20,separatorRows:[20]}],waves:maxWavePayload.waves.map((wave,index)=>index===0?{...wave,value:'11:15 (16)'}:wave)};
  const overflowWavePreflight=morningSheetsPreflight(overflowWavePayload);
  if(overflowWavePreflight.ready||!overflowWavePreflight.checks.some(check=>!check.ok&&/15|capacity|route row/i.test(check.label+' '+check.detail)))throw new Error('Client preflight must reject a 16th route before Google Sheets is called');
  const preflightHtml = morningSheetsPreflightHtml(payload);
  if (!preflightHtml.includes('Preflight ready') || !preflightHtml.includes('A–M write scope only') || !preflightHtml.includes('Columns N+ stay untouched') || !preflightHtml.includes('Every row has 13 columns') || !preflightHtml.includes('Black dividers are real rows') || !preflightHtml.includes('All 6 wave slots ready') || !preflightHtml.includes('Wave/Pad merge map ready') || !preflightHtml.includes('Target cell A3')) throw new Error('Morning Sheets preflight UI missing required checks');
  const proof = morningSheetsHandoffProof(payload);
  if (!proof.ready || proof.rows !== payload.rows.length || proof.visibleRows !== payload.rows.length || proof.range !== ('A3:M' + (payload.rows.length + 2)) || proof.dividers !== payload.rowTypes.filter(type=>type==='separator').length) throw new Error('Morning Sheets handoff proof should match visible rows and all divider rows to connector payload');
  ...
  const whipPayload=morningWhiparoundOnlyPayload();
  if(whipPayload.mode!=='whiparound-only'||!whipPayload.updates.length||whipPayload.updates.some(update=>typeof update.preWhip!=='boolean'||typeof update.postWhip!=='boolean')||Object.keys(whipPayload).some(key=>['rows','headers','sections'].includes(key)))throw new Error('Whiparound-only Google payload must contain only route checkbox updates');
  if(!action.toString().includes('send-whiparound-to-sheets')||!sendWhiparoundChecksToGoogleSheets.toString().includes("mode!=='whiparound-only'")||!sendWhiparoundChecksToGoogleSheets.toString().includes('PRE-WHIP and POST-WHIP'))throw new Error('Independent Whiparound Google send is missing');
  ...
  if (JSON.stringify(normalizedPayloadRows) !== JSON.stringify(compactVisibleRows)) throw new Error('Compact connector payload must match mapped values from the visible A-V template');
  ...
  if (!script.includes('function onOpen') || !script.includes("createMenu('RelayOps')") || !script.includes('relayOpsConnectorStatus') || !script.includes('Validate template layout') || !script.includes('Run demo preflight')) throw new Error('Apps Script should add a RelayOps menu for safe install verification');
  ...
  const rtsConnectorContracts=["payload.mode === 'rts-only'",'function validateRelayOpsRtsPayload','function writeRelayOpsRtsOnly','function relayOpsRouteIndex','sheet.getRange(record.row, 21).setValue(update.plannedRts)','function writeRelayOpsWaveLabels','cell.clearContent()','cell.setValue(String(wave.value))','Wave footer verification failed'];
  ...
  if (!script.includes('const invalidTime = Boolean(fixedLayout.timeRow)') || !script.includes('const invalidSeparator = Boolean(fixedLayout.separatorRow)')) throw new Error('Apps Script must accept fixed ADHOC/HELPERS sections without fake time rows while validating real wave anchors');
  ...
  if (!script.includes('function relayOpsTemplateLayout') || !script.includes('const neededRows = RELAYOPS_LAST_ROW') || !script.includes('hasEnoughRows: sheet.getMaxRows() >= neededRows') || !script.includes('hasEnoughColumns: sheet.getMaxColumns() >= RELAYOPS_TEMPLATE_COLS')) throw new Error('Apps Script should verify the fixed 142-row Google Sheet template capacity before writing');
  if (script.includes('insertRowsAfter') || script.includes('insertColumnsAfter')) throw new Error('Fixed 142-row connector must not mutate the authoritative template geometry');
  if (!script.includes('const result = writeRelayOpsMorningSheet(payload)') || !script.includes('sheet: result.sheetName') || !script.includes('startCell: result.startCell') || !script.includes('writeRange: result.writeRange') || !script.includes('writtenRange: result.writtenRange') || !script.includes("writtenRange: 'A3:V142'") || !script.includes("lastCell: 'V142'")) throw new Error('Apps Script confirmation should report the exact A3:V142 range');
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || MORNING_CONNECTOR_BUILD !== '2026-08-03-six-wave-142-row-layout' || !testMorningSheetsConnector.toString().includes('activeMorningConnectorBuild()') || !testMorningSheetsConnector.toString().includes('activeMorningStationCode()') || !testMorningSheetsConnector.toString().includes('Connector deployment does not match')) throw new Error('Connector test should verify the active station build while preserving the DJT6 six-wave deployment');
  ...
  if (!selectedSheetHtml.toString().includes('rowspan=') || !selectedSheetHtml.toString().includes('mergedThrough') || !sheetSelectionBounds.toString().includes('state.copyMode?null')) throw new Error('Drag selection should cross wave sections and preserve merged Wave/Pad cells in rich clipboard HTML');
