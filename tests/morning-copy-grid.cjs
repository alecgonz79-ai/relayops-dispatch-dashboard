...
const checks = `
  ...
  if (!html.includes('ONE-CLICK OPS LOG TRANSFER') || !html.includes('Send Morning Sheet to Ops Log') || !html.includes('Connect Ops Log once') || !html.includes('Copy fallback') || !html.includes('Handoff proof') || !html.includes('Google range') || !html.includes('Connector rows') || !html.includes('Ready for copy fallback') || !html.includes('Import / route source') || !html.includes('Visible rows = payload') || !html.includes('Exact connector')) throw new Error('Exact-format handoff UI should prioritize the Ops Log connector and show row-proof details');
