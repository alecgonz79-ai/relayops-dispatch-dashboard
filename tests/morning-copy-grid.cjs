...
const checks = `
  ...
  if (!html.includes('Filtered waves') || !html.includes('Automatic check') || !html.includes('Google Ops Log') || !html.includes('Connect Google Sheet') || !html.includes('Dry run happens before every send') || !html.includes('Copy fallback') || !html.includes('Handoff proof') || !html.includes('Google range') || !html.includes('Connector rows') || !html.includes('Ready for copy fallback') || !html.includes('Import / route source') || !html.includes('Visible rows = payload') || !html.includes('Exact connector')) throw new Error('Exact-format handoff UI should prioritize the filtered-waves Google bridge and show row-proof details');
  ...
  if (!html.includes('date tab must already exist')) throw new Error('Dated Google tab safety guidance missing');
  ...
  if (!script.includes('freezeRelayOpsHeader(sheet)') || !script.includes('getMergedRanges()') || !script.includes('if (range.getRow() === 1) range.breakApart()') || !script.includes('return sheet.getFrozenRows()') || script.includes('sheet.setFrozenRows(1)') || !script.includes('setValues([headers])') || !script.includes("sheet.setRowHeight(1, 28)") || !script.includes("sheet.getRange(1, 9).setBackground('#050505')") || !script.includes("sheet.getRange(1, 13).setBackground('#b4a7d6')")) throw new Error('Apps Script should unmerge only the header boundary, preserve the existing freeze setting, and restore A-M formatting');
  if (!script.includes('target.clearFormat()') || !script.includes("rowType === 'time'") || !script.includes('.setFontSize(10).setTextRotation(0)')) throw new Error('Apps Script should clear inherited A3:M formatting and force wave-time rows back to normal horizontal text');
  ...
  if (!connectorHtml.includes('GOOGLE SHEETS CONNECTOR') || !connectorHtml.includes('COPY APPS SCRIPT CODE') || !connectorHtml.includes('Copy setup checklist') || !connectorHtml.includes('Copy verify checklist') || !connectorHtml.includes('Download .gs file') || !connectorHtml.includes('Preflight ready') || !connectorHtml.includes('Handoff proof') || !connectorHtml.includes('Row-by-row audit') || !connectorHtml.includes('First write row') || !connectorHtml.includes('Last write row') || !connectorHtml.includes('No row mismatches') || !connectorHtml.includes('Live Google test target') || !connectorHtml.includes('Expected finish') || !connectorHtml.includes('Frozen row') || !connectorHtml.includes('Columns N+') || !connectorHtml.includes('black divider rows still numbered') || !connectorHtml.includes('Connector rows') || !connectorHtml.includes('A–M write scope only') || !connectorHtml.includes('Columns N+ stay untouched') || !connectorHtml.includes('writes A3:M only') || !connectorHtml.includes('Every row has 13 columns') || !connectorHtml.includes(MORNING_APPS_SCRIPT_URL) || !connectorHtml.includes('Test connector') || !connectorHtml.includes('Dry run') || !connectorHtml.includes('Send to Google Sheet') || !connectorHtml.includes('Advanced transfer preview — do not paste into Apps Script') || !connectorHtml.includes('It is not Apps Script code') || connectorHtml.includes('Copy payload JSON')) throw new Error('Morning Sheets connector modal missing or JSON/script distinction is unclear');
  if (!${JSON.stringify(connectorFile)}.includes('function onOpen') || !${JSON.stringify(connectorFile)}.includes("createMenu('RelayOps')") || !${JSON.stringify(connectorFile)}.includes('Validate template layout') || !${JSON.stringify(connectorFile)}.includes('function relayOpsValidateTemplate') || !${JSON.stringify(connectorFile)}.includes('function validateRelayOpsMorningPayload') || !${JSON.stringify(connectorFile)}.includes('Write range must be A3:M') || !${JSON.stringify(connectorFile)}.includes('payload.dryRun') || !${JSON.stringify(connectorFile)}.includes('function relayOpsTemplateLayout') || !${JSON.stringify(connectorFile)}.includes('function ensureRelayOpsTemplateCapacity') || !${JSON.stringify(connectorFile)}.includes('function freezeRelayOpsHeader') || !${JSON.stringify(connectorFile)}.includes('getMergedRanges()') || !${JSON.stringify(connectorFile)}.includes('freezeRelayOpsHeader(sheet)') || !${JSON.stringify(connectorFile)}.includes('function findRelayOpsMorningSheet') || !${JSON.stringify(connectorFile)}.includes('ok: false') || !${JSON.stringify(connectorFile)}.includes('LockService.getDocumentLock()') || !${JSON.stringify(connectorFile)}.includes('function doPost') || !${JSON.stringify(connectorFile)}.includes('writeRelayOpsMorningSheet') || !${JSON.stringify(connectorFile)}.includes("rowType === 'separator'") || !${JSON.stringify(connectorFile)}.includes('sheet: result.sheetName')) throw new Error('Permanent Apps Script connector file missing required behavior');
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:M') || !testMorningSheetsConnector.toString().includes('2026-07-10-dated-tab') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify the A3:M range and current dated-tab deployment build');
  ...
  if (!verifyChecklist.includes('RelayOps Google Sheet Send Verification') || !verifyChecklist.includes('Expected write range: A3:M') || !verifyChecklist.includes('Expected last cell: M') || !verifyChecklist.includes('Protected write scope: A3:M only') || !verifyChecklist.includes('Row 1 should stay frozen') || !verifyChecklist.includes('columns N and beyond were not changed') || !verifyChecklist.includes('Range A3:M') || !verifyChecklist.includes('Last cell M') || !verifyChecklist.includes('Sent — verify in Google Sheet')) throw new Error('Morning Sheets verification checklist missing required checks');
