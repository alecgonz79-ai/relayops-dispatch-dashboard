...
const checks = `
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || !testMorningSheetsConnector.toString().includes('2026-07-12-values-only-transfer') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify OPS LOG 2026 values-only deployment');
