...
const checks = `
  ...
  const rtsConnectorContracts=["payload.mode === 'rts-only'",'function validateRelayOpsRtsPayload','function writeRelayOpsRtsOnly','function relayOpsRouteIndex','sheet.getRange(record.row, 21).setValue(update.plannedRts)','function writeRelayOpsWaveLabels','sheet.getRange(wave.layout.timeRow, 1).setValue'];
  ...
  if (!testMorningSheetsConnector.toString().includes('A3:V') || !testMorningSheetsConnector.toString().includes('2026-07-20-all-wave-labels') || !testMorningSheetsConnector.toString().includes('Connector deployment is outdated')) throw new Error('Connector test should verify the current OPS LOG 2026 all-wave-label deployment');
